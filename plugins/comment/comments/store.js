...
export const requestComments = (article) => async dispatch => {
    ...
    dispatch({type: actionTypes.RECEIVE_COMMENTS, comments: res.comments})
};
...
export const submitReply = (articleId, name, email, content, replyCommentId) => async dispatch => {
    dispatch({type: actionTypes.SUBMIT_REPLY});
    const res = await request({
        url: `article/${articleId}/comment${replyCommentId ? '/' + replyCommentId : ''}`,
        method: 'POST',
        data: {
            articleId: articleId,
            name: name,
